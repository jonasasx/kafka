...
class DeleteMembersOfConsumerGroupTest extends ConsumerGroupCommandTest {
  ...
  def testDeleteAllMembersFromGroup(): Unit = {
    ...
    val cgcArgs = Array("--bootstrap-server", bootstrapServers(), "--remove-members", "--group", group, "--all-members")
  ...
  def testDeleteAllMembersFromGroupWithMissingGroup(): Unit = {
    ...
    val cgcArgs = Array("--bootstrap-server", bootstrapServers(), "--remove-members", "--group", missingGroup, "--all-members")
    val service = getConsumerGroupService(cgcArgs)
  ...
  def testDeleteAllMembersFromGroupWithNoMembers(): Unit = {
    ...
    // run one consumer in the group
    addConsumerGroupExecutor(numConsumers = 1)
    val cgcArgs = Array("--bootstrap-server", bootstrapServers(), "--remove-members", "--group", group, "--member", missingMember)
    val service = getConsumerGroupService(cgcArgs)
